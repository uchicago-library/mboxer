...
dev-install: dune-install
	install -m 555 $(OPAM_SWITCH_PREFIX)/bin/mboxer ~/bin
.PHONY: home-install
...
publish: build mounts
	echo 'url { src:' $$(cat mboxer.opam | grep dev-repo | awk '{ print $$2 }') '}' >> mboxer.opam
	make -C /data/web/dldc/opam add NAME=mboxer OPAM=$$PWD/mboxer.opam
.PHONY: publish
